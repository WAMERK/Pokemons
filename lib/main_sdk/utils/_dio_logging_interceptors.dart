...
import 'package:pokemon/main_sdk/utils/mall_sdk.dart';
...
class DioLoggingInterceptors extends InterceptorsWrapper {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    ...
    options.queryParameters
        .forEach((k, v) => debugPrint('$k: $v', wrapWidth: 1024));
  ...
  Future<void> onError(
    ...
    debugPrint(
        '<-- ${dioError.message} ${((dioError.requestOptions.baseUrl + dioError.requestOptions.path))}',
        wrapWidth: 1024);
    debugPrint(
    ...
    if (oldAccessToken != null &&
        responseCode == 401 &&
        !dioError.requestOptions.path.contains('token')) {
